...
color_default = lambda x, y, z: [
    b + (v * random() * 2) - v for b, v in zip(color_base, color_var)]
color_sun = (1.5, 1.1, 1)
...
def print_screen(rows, columns, screen, stdout):
    ...
    for y in range(0, rows, 1):
        for x in range(0, columns, 1):
            c = map_color_to_char(screen[y][x],y,x)
            print_string += c
...
def map_color_to_char(color, y, x):
    try:
        r, g, b = map((lambda c: max(0,min(1,(c**gamma_correction)))), color)
        final_color = (r+g+b)/3.0   # averge to get grayscale
        gradient_raw = (final_color*len(gradient))
        gradient_index = int(gradient_raw)
        # error = gradient_raw - gradient_index
        # if [0, 8, 2, 10, 12, 4, 14, 6, 3, 11, 1, 9, 15, 7, 13, 5]\
            #    [2 * (int(x) % 4) + (int(y) % 4)] < error * 16:
        #     gradient_index += 1

        return gradient[max(0,min(len(gradient)-1,gradient_index))]
    except:
        return "?"
